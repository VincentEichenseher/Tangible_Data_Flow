...
# important params:
# modify as needed
update_interval = 500 # update interval in ms
...
if json_data is None:
    with open("../node_data.json","r") as file_in:
        json_data = json.load(file_in)
    for item in json_data["nodes"]:
        item["is_active"] = False
    with open("./node_data.json","w") as file_out:
        file_out.write(json.dumps(json_data))
    with open("./stack_data.json","w") as file_out_numerodos:
        file_out_numerodos.write(json.dumps(json_data))
...
def main():
    ...
    server = osc_server.BlockingOSCUDPServer((args.ip, args.port), dispatch)
...
#        print("cant open node_data")
        pass
    with open("./stack_data.json","r") as stack_fh:
      stack_data = json.load(stack_fh)
    for element in json_data["nodes"]:
        ...
        if element["is_active"] is True and id_as_string not in list(elements_added):
            ...
            for key in new_values.keys():
                if el_id in [4,5,7]:
                    value_types = json_data["nodes"][el_id]["value_types"]
                    possible_types = value_types[key]
                    index = possible_types.index(new_values[key])
                    ui[key].setCurrentIndex(index)
                elif el_id in [0,1,2,3,6]:
